...
/** Sign in with Apple requires the [ASAuthorizationAppleIDRequest.nonce] sent to Apple to be the
 *  SHA-256 hash of a raw nonce, while the SAME raw (unhashed) nonce goes to Firebase — so
 *  Firebase can hash it again and confirm it matches the value embedded in Apple's identity
 *  token, proving the token wasn't replayed. A pure-Kotlin implementation avoids pulling in a
 *  CommonCrypto/CryptoKit cinterop just for one hash. */
...
private fun sha256(data: ByteArray): ByteArray {
    ...
    val zeroPad = ((56 - withMarker.size % 64) + 64) % 64
    val message = withMarker + ByteArray(zeroPad) + ByteArray(8).also { lenBytes ->
        for (i in 0 until 8) lenBytes[7 - i] = ((bitLength shr (i * 8)) and 0xFF).toByte()
    }
    ...
    for (chunkStart in message.indices step 64) {
        ...
        for (i in 0 until 16) {
            ...
            w[i] = ((message[base].toUInt() and 0xFFu) shl 24) or
                ((message[base + 1].toUInt() and 0xFFu) shl 16) or
                ((message[base + 2].toUInt() and 0xFFu) shl 8) or
                (message[base + 3].toUInt() and 0xFFu)
        }
