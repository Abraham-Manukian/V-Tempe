...
**✅ ЗАВЕРШЕНА (2026-07-13).** Все пункты 1.1–1.4 сделаны, каждый — отдельным коммитом, прогнан через ревью Fable 5 (несколько раундов на 1.3 и особенно 1.4 — там ревью нашло и я исправил реальный баг с утечкой in-flight записи в BundleCache, см. коммит 1.4). Известное, сознательно не исправленное: (а) `Flow.catch` в `HomePresenter.kt` терминален — Retry не может пересоздать упавший `observeWorkouts()`/`observePlan()`, только `ensureCoachData()`; (б) в `AiService.fetchBundle()` код ПЕРЕД `try`-блоком (`buildBundlePrompt`, `safeLocale` и т.д.) всё ещё может утечь in-flight запись при падении — это баг, существовавший ДО сегодняшних изменений, не новый. Оба — задокументированы в соответствующих коммитах, не исправлялись сегодня. Коммиты: `b11958c`, `4f470e8`, `35d24bf`.
